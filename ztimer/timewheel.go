...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	tw := &TimeWheel{
		...
		scales:     scales,
		maxCap:     maxCap,
		timerQueue: make(map[int]map[uint64]*Timer, scales),
	}
	// 初始化map
	for i := 0; i < scales; i++ {
		tw.timerQueue[i] = make(map[uint64]*Timer, maxCap)
	}

	fmt.Println("Init timerWhell name = ", tw.name, " is Done!")
	return tw
}
...
/*
	...
	如果当前的timer的超时时间间隔 小于一个刻度 :
					如果没有下一轮时间轮
*/
func (tw *TimeWheel) addTimer(tID uint64, t *Timer, forceNext bool) error {
	defer func() error {
...
func (tw *TimeWheel) AddTimer(tID uint64, t *Timer) error {
	tw.Lock()
	defer tw.Unlock()
...
func (tw *TimeWheel) RemoveTimer(tID uint64) {
	tw.Lock()
	defer tw.Unlock()
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	fmt.Println("Add timerWhell[", tw.name, "]'s next [", next.name, "] is succ!")
}
...
func (tw *TimeWheel) run() {
	for {
		...
		tw.timerQueue[tw.curIndex] = make(map[uint64]*Timer, tw.maxCap)
		...
		tw.timerQueue[(tw.curIndex+1)%tw.scales] = make(map[uint64]*Timer, tw.maxCap)
...
func (tw *TimeWheel) GetTimerWithIn(duration time.Duration) map[uint64]*Timer {
	...
	defer leaftw.Unlock()
	// 返回的Timer集合
	timerList := make(map[uint64]*Timer)
