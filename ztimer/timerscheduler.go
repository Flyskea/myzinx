...
func (ts *TimerScheduler) CreateTimerAt(df *DelayFunc, unixNano int64) (uint64, error) {
	ts.Lock()
	defer ts.Unlock()
...
func (ts *TimerScheduler) CreateTimerAfter(df *DelayFunc, duration time.Duration) (uint64, error) {
	ts.Lock()
	defer ts.Unlock()
...
// CancelTimer 删除timer
func (ts *TimerScheduler) CancelTimer(tID uint64) {
	ts.Lock()
	defer ts.Unlock()
